        ...
        <table class="w-full text-sm border">
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-gray-50 dark:hover:bg-zinc-800">
                    ...
                    @php
                        $totalStock = $product->variants->count() ? $product->variants->sum('stock') : $product->stock;
                    @endphp
                    <td class="p-3">
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-primary h-2 rounded-full"
                                 style="width: {{ min(100, ($totalStock / max($totalStock, 1)) * 100) }}%"></div>
                        </div>
                        <span class="text-xs">{{ $totalStock }} in stock</span>
                    </td>
