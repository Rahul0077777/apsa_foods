...
<main class="bg-[#f6f8f6] dark:bg-[#0f1f0f] min-h-screen py-12">
...
<div>
    ...
    <div class="flex gap-4 mt-6">
        <img src="{{ asset('storage/'.$image) }}"
             ...
             class="w-20 h-20 rounded-xl object-cover border-2 border-green-500 p-1 cursor-pointer">
...
<div>
    ...
    <h1 class="text-5xl md:text-6xl italic tracking-tight text-gray-900 dark:text-white"
    ...
    <div class="grid grid-cols-2 gap-6 mb-8">
        ...
        <div>
            <label class="text-xs font-semibold text-gray-500">SELECT VOLUME</label>
            <select class="w-full mt-2 border rounded-xl px-4 py-3 bg-gray-50">
                <option>160ml Bottle</option>
                <option>200ml Bottle</option>
                <option>250ml Bottle</option>
                <option>500ml Bottle</option>
            </select>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-20">
        ...
        <div class="{{ $index % 2 != 0 ? 'md:order-2' : '' }}">
            <a href="{{ route('product.show',$rel->slug) }}">
                <img src="{{ asset('storage/'.$rel->image) }}"
                     class="w-full h-[320px] md:h-[390px] object-contain rounded-xl shadow-md bg-white p-6">
            </a>
        </div>
        ...
        <div class="text-center md:text-left
            ...
            <div class="flex flex-col items-center
                ...
                {{ $index % 2 != 0 
                    ? 'sm:justify-end sm:space-x-reverse' 
                    : 'sm:justify-start' }}">
                ...
                {{-- Button --}}
                @if($rel->status)
                <form action="{{ route('cart.add', $rel->id) }}" method="POST" class="w-full sm:w-auto">
                    @csrf
