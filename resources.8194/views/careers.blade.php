...
<div class="relative flex min-h-screen w-full flex-col overflow-x-hidden">
                    ...
                    <div class="@[480px]:p-4">
                        <div class="relative flex min-h-[400px] flex-col gap-6 overflow-hidden rounded-xl bg-cover bg-center bg-no-repeat p-8 text-center items-center justify-center"
                            ...
                            <div class="flex flex-col gap-4">
                                <h1 class="text-white text-4xl font-black md:text-5xl">Careers & Opportunities</h1>
                                <p class="mx-auto max-w-lg text-white opacity-90">
                                    Join the Fresh Revolution. We are looking for passionate people to help us blend the
                ...
                <div class="flex flex-col gap-8 px-4 py-6">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                        ...
                        <div
                            ...
                            <h3 class="text-text-main text-base font-bold dark:text-white">Fresh Perks</h3>
                            <p class="text-text-secondary text-sm dark:text-[#8bc48b]">Daily smoothie allowance, product
                                discounts, and weekly team lunches.</p>
                        </div>
                        <!-- Perk 2 -->
                        <div
                            class="group flex flex-col gap-3 rounded-xl border border-accent-light bg-white p-6 shadow-sm transition-all hover:border-primary/50 hover:shadow-md dark:bg-[#1a2e1a] dark:border-[#2a4e2a]">
                            ...
                            <h3 class="text-text-main text-base font-bold dark:text-white">Health Benefits</h3>
                            <p class="text-text-secondary text-sm dark:text-[#8bc48b]">Comprehensive medical, dental,
                                and vision coverage for you and your family.</p>
                        </div>
                        <!-- Perk 3 -->
                        <div
                            class="group flex flex-col gap-3 rounded-xl border border-accent-light bg-white p-6 shadow-sm transition-all hover:border-primary/50 hover:shadow-md dark:bg-[#1a2e1a] dark:border-[#2a4e2a]">
                            ...
                            <h3 class="text-text-main text-base font-bold dark:text-white">Flexible Hours</h3>
                            <p class="text-text-secondary text-sm dark:text-[#8bc48b]">Hybrid work options for corporate
                                roles and flexible scheduling for retail.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full px-4 md:px-10 lg:px-40">
            <div class="mx-auto flex w-full flex-col gap-6 px-4">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <h2 class="text-text-main text-2xl font-bold">Open Roles</h2>
                    <!-- Search Input -->
                ...
                <div class="flex flex-wrap gap-2 pb-4">
                    <button
                        class="rounded-full bg-primary px-4 py-1.5 text-sm font-semibold text-text-main shadow-sm transition-colors">
                        All Departments
                    </button>
                    <button
                        class="rounded-full bg-accent-light px-4 py-1.5 text-sm font-medium text-text-main transition-colors hover:bg-primary/20">
                        Retail Operations
                    </button>
                    <button
                        class="rounded-full bg-accent-light px-4 py-1.5 text-sm font-medium text-text-main transition-colors hover:bg-primary/20">
                ...
                <div class="lg:col-span-7 flex flex-col gap-4">
                    ...
                    <details
                        ...
                        <div class="border-t px-5 py-4">
                            ...
                            <button data-job="{{ $job->id }}" class="applyBtn mt-3 bg-primary px-4 py-2 rounded">
                                Apply
                            </button>
                        </div>
                    </details>
                ...
                <div class="lg:col-span-5">
                    <div
                        ...
                        <p class="mb-6 text-sm text-text-secondary dark:text-[#8bc48b]">Take the first step towards
                            a fresh career.</p>
                        <form action="{{ route('career.apply') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4">
    @csrf

    <!-- Name -->
    <div>
        <label class="mb-1 block text-sm font-medium text-text-main dark:text-gray-300">Full Name</label>
        <input
            name="name"
    ...
    <!-- Email -->
    <div>
        <label class="mb-1 block text-sm font-medium text-text-main dark:text-gray-300">Email Address</label>
        <input
            name="email"
            class="w-full rounded-lg border-accent-light bg-background-light p-2.5 text-sm text-text-main placeholder-text-secondary focus:border-primary focus:ring-primary dark:bg-[#102210] dark:border-[#2a4e2a]"
    ...
    <!-- Role Selection -->
    <div>
        <label class="mb-1 block text-sm font-medium text-text-main dark:text-gray-300">Position</label>
        <div class="relative">
            <select
                name="career_opening_id"
    ...
    <!-- LinkedIn -->
    <div>
        <label class="mb-1 block text-sm font-medium text-text-main dark:text-gray-300">LinkedIn / Portfolio URL</label>
        <input
            name="portfolio"
            class="w-full rounded-lg border-accent-light bg-background-light p-2.5 text-sm text-text-main placeholder-text-secondary focus:border-primary focus:ring-primary dark:bg-[#102210] dark:border-[#2a4e2a]"
    ...
    <!-- Resume Upload -->
    <div class="mt-2">
        <label class="mb-1 block text-sm font-medium text-text-main dark:text-gray-300">Resume/CV</label>
        <div
            ...
            <span class="material-symbols-outlined mb-2 text-3xl text-text-secondary">cloud_upload</span>
            ...
            <input name="resume" accept=".pdf,.docx,.doc"
                class="absolute inset-0 h-full w-full opacity-0 cursor-pointer" type="file" required />
        </div>
    </div>
